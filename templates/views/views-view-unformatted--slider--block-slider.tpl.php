...
<div id="slideshow" class="banner-container main-slider" style="position: relative;">
  <div class="banner">
    <ul>
    <?php $rs_effect='fade'; ?>
    <?php foreach ($rows as $id => $row): ?>
      ...
      <?php 
        ...
        $title = $node->title;
        $field_slideshow_entry_image = field_get_items('node', $node, 'field_slider_imagen');
        $field_slider_link  = field_get_items('node', $node, 'field_slider_link');

        $image    = image_style_url('slideshow', $field_slideshow_entry_image[0]['uri']); 
        $path     = ($field_slider_link) ? $field_slider_link[0]['safe_value'] : false;
